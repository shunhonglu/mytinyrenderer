...
struct GouraudShader : public IShader {
  GouraudShader(
      const mat<4, 4, float>& u_M = View * ModelMat,
  ...
  virtual Vec4f vertex(int iface, int nthvert) {
    varying_uv.set_col(
        nthvert, model->uv(iface, nthvert));  // read the uv from .obj file

    Vec4f gl_Vertex = embed<4>(
        model->vert(iface, nthvert));  // read the vertex from .obj file
    gl_Vertex =
        transformation(gl_Vertex);  // transform it to screen coordinates

    Vec4f normal = uniform_MIT * toVec4f(model->normal(iface, nthvert), 0.f);
    varying_intensity[nthvert] = std::max(
        0.f, proj<3>(normal).normalize() *
                 light_dir.normalize());  // get diffuse lighting intensity
    return gl_Vertex;
  }
  ...
  virtual bool fragment(Vec3f bar, TGAColor& color) {
    ...
    float intensity = varying_intensity * bar /
                      sum;  // interpolate intensity for the current pixel
...
struct PhongShader : public IShader {
  PhongShader(
      float am = 5.f, float kd_ = 1.f, float ks_ = 0.6f,
      const mat<4, 4, float>& u_M = View * ModelMat,
      const mat<4, 4, float>& u_MIT = (View * ModelMat).invert_transpose())
      : ambient(am),
        kd(kd_),
  ...
  virtual bool fragment(Vec3f bar, TGAColor& color) {
    ...
    for (int i = 0; i < 3; i++)
      color[i] =
          std::min<float>(ambient + c[i] * (kd * diffuse + ks * specular), 255);
    return false;  // no, we do not discard this pixel
  }
};
...
int main(int argc, char** argv) {
  ...
  sample_list_color =
      vector<vector<TGAColor>>(width * height, vector<TGAColor>(4));

  for (int i = 0; i < width; ++i) {
    for (int j = 0; j < height; ++j) zbuffer.set(i, j, TGAColor(255));
  }
  ...
  for (int i = 0; i < model->nfaces(); i++) {
    ...
    triangle(screen_coords, *s, image, zbuffer, sample_list, sample_list_color,
             near, far);
  }
...
//     image.  flip_vertically(); // to place the origin in the bottom left
//     corner of the image image.  write_tga_file("twocube.tga"); return 0;
// }